...
resource "aws_instance" "hyperflowmaster" {
  ...
  user_data = "#!/bin/bash\necho ECS_CLUSTER=${var.ecs_cluster_name} >> /etc/ecs/ecs.config && docker run --net=host -e DISK_DEVICE='xvda1' -e INTERFACE='eth0' -e METRIC_COLLECTOR='${local.influx_db_url}'  ${var.ec2_status_reporter} &"

  provisioner "local-exec" {
    command = "echo ${aws_instance.hyperflowmaster.public_dns} > master-dns.txt"
  }
}
